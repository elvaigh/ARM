...
char* getCharValue(vtTempMsg *Buffer)
{
	char *ptrC = (char*) Buffer->buf;
	return(*ptrC);
}
...
static portTASK_FUNCTION( vi2cTempUpdateTask, pvParameters )
{
	...
	sprintf(lcdBuffer,"      Time");
	if (SendLCDPrintMsg(lcdData,strnlen(lcdBuffer,vtLCDMaxLen),lcdBuffer,9,portMAX_DELAY) != pdTRUE) {
						VT_HANDLE_FATAL_ERROR(0);
	}
	sprintf(lcdBuffer," Voltage");
	if (SendLCDPrintMsgVert(lcdData,strnlen(lcdBuffer,vtLCDMaxLen),lcdBuffer,0,portMAX_DELAY) != pdTRUE) {
						VT_HANDLE_FATAL_ERROR(0);
 	}*/
	#endif
	...
	for(;;)
	{
		...
		case vtI2CMsgTypeTempRead3: {
				...
				buff[buffCount] = voltage;
				buffCount++; 
				 
				
				if ((lcdData != NULL) && (buffCount == 100)) {
					if (SendLCDPixelBuff(lcdData,xStart,buff,portMAX_DELAY) != pdTRUE) {
						VT_HANDLE_FATAL_ERROR(0);
					}
					xStart = xStart+100;
					if(xStart >= 320)
					{
						xStart = xStart -320 + 40;
					}
					buffCount = 0;
				}
			#else
				...
				sprintf(lcdBuffer,"     Value = %c",(char)(voltage));
				//sprintf(lcdBuffer,"     Value = %d",lrint(voltage));
				if (lcdData != NULL) {
